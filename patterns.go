...
var RegexPatterns = map[string]string{
	...
	"Configuration API Key":               `"CONFIGURATION_API_KEY"\s*:\s*"([A-Za-z0-9_-]+)"`,
	...
	"Basic Auth":     `(?i)basic\s+[A-Za-z0-9=:_\+\/-]{5,100}`,
	...
	"PGP PRIVATE KEY":     `-----BEGIN PGP PRIVATE KEY BLOCK-----[\s\S]+?-----END PGP PRIVATE KEY BLOCK-----`,
	...
	"ENV Secrets":       `(?i)(?:api[_\- ]?key|access[_\- ]?token|client[_\- ]?secret|secret|refresh[_\- ]?token)[=:]['"]?([A-Za-z0-9\-_\/+=\.]{8,})['"]?`,
...
// --- Regex for links and paths ---
var (
	linkRegex = regexp.MustCompile(`(?i)(?:href|src|url|action)\s*=\s*["']([^"'>\s]+)["']|fetch\(["']([^"']+)["']\)`)
	pathRegex = regexp.MustCompile(`(?m)(?:["'\s]|^)(~\/[^\s"'<>]+|\/[A-Za-z0-9._\-/]+|(?:\.\.?\/)[A-Za-z0-9._\-/]+)(?:["'\s]|$)`)
)
